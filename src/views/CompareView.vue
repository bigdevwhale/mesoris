...
<template>
  <div class="max-w-7xl mx-auto px-4 py-10">
    ...
    />
    <h1 class="text-display-lg mb-2">Compare Dinosaurs</h1>
    <p class="text-body-lg mb-10">Pick two dinosaurs and see how they stack up side by side.</p>

    <!-- Selectors -->
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
      <div class="bg-[var(--color-bg-elevated)] border border-[var(--glass-border)] rounded-[var(--radius-xl)] p-6">
        <h3 class="text-heading-md mb-4">Dinosaur A</h3>
        <BaseSelect
        ...
        />
        <div v-if="store.dinosaurA" class="mt-4 flex items-center gap-4">
          ...
          <div>
            <div class="font-semibold text-[var(--color-text-primary)]">{{ store.dinosaurA.name }}</div>
            <div class="text-sm text-[var(--color-text-tertiary)]">{{ store.dinosaurA.era }} &middot; {{ store.dinosaurA.diet }}</div>
          </div>
        </div>
      </div>

      <div class="bg-[var(--color-bg-elevated)] border border-[var(--glass-border)] rounded-[var(--radius-xl)] p-6">
        <h3 class="text-heading-md mb-4">Dinosaur B</h3>
        <BaseSelect
        ...
        />
        <div v-if="store.dinosaurB" class="mt-4 flex items-center gap-4">
          <div class="w-16 h-16 rounded-full overflow-hidden shrink-0">
            <DinoCardImage
          ...
          <div>
            <div class="font-semibold text-[var(--color-text-primary)]">{{ store.dinosaurB.name }}</div>
            <div class="text-sm text-[var(--color-text-tertiary)]">{{ store.dinosaurB.era }} &middot; {{ store.dinosaurB.diet }}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="store.bothSelected" class="flex gap-3 mb-8">
      <BaseButton variant="ghost" size="sm" icon="arrow-left-right" @click="store.swap()">Swap</BaseButton>
      <BaseButton variant="ghost" size="sm" icon="x" @click="store.clear()">Clear</BaseButton>
    </div>

    <!-- Comparison results -->
    <div v-if="comparison" class="space-y-6">
      <div v-for="stat in comparison.stats" :key="stat.label" class="bg-[var(--color-bg-elevated)] border border-[var(--glass-border)] rounded-[var(--radius-lg)] p-5">
        <div class="flex items-center justify-between mb-3">
          <span class="text-sm font-semibold text-[var(--color-text-primary)]">{{ stat.label }}</span>
          <span class="text-xs text-[var(--color-text-tertiary)]">{{ stat.unit }}</span>
        </div>

        <!-- Bar A -->
        <div class="flex items-center gap-3 mb-2">
          <span class="w-28 text-right text-sm font-medium text-[var(--color-text-secondary)] shrink-0 truncate">{{ comparison.dinosaurA.name }}</span>
          <div class="flex-1">
        ...
        <div class="flex items-center gap-3">
          <span class="w-28 text-right text-sm font-medium text-[var(--color-text-secondary)] shrink-0 truncate">{{ comparison.dinosaurB.name }}</span>
          <div class="flex-1">
            <div class="flex items-center gap-2">
              <div class="flex-1 h-6 rounded-full bg-[var(--color-bg-overlay)] overflow-hidden">
                <div
      ...
      <div class="bg-[var(--color-bg-elevated)] border border-[var(--glass-border)] rounded-[var(--radius-xl)] p-8">
        <h3 class="text-heading-md mb-6 text-center">Height Comparison</h3>
        <div class="flex items-end justify-center gap-8 sm:gap-16">
          ...
          <div class="flex flex-col items-center">
            <div
              ...
              :style="{ height: `${Math.min(comparison.dinosaurA.dimensions.heightMeters * 30, 240)}px` }"
            />
            <div class="text-xs text-[var(--color-text-secondary)] mt-2 text-center">
              <div class="font-semibold">{{ comparison.dinosaurA.name }}</div>
              <div>{{ comparison.dinosaurA.dimensions.heightMeters }}m</div>
            </div>
          </div>
          ...
          <div class="flex flex-col items-center">
            <div
              ...
              :style="{ height: `${Math.min(comparison.dinosaurB.dimensions.heightMeters * 30, 240)}px` }"
            />
            <div class="text-xs text-[var(--color-text-secondary)] mt-2 text-center">
              <div class="font-semibold">{{ comparison.dinosaurB.name }}</div>
              <div>{{ comparison.dinosaurB.dimensions.heightMeters }}m</div>
            </div>
          </div>
        </div>
    ...
    <div v-if="!store.dinoIdA && !store.dinoIdB" class="text-center py-20">
      ...
      <h2 class="text-heading-md mb-2">Select Two Dinosaurs</h2>
      <p class="text-body-md text-[var(--color-text-secondary)]">Pick dinosaurs from the dropdowns above to start comparing.</p>
    </div>
  </div>
</template>
